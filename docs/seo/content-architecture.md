...
Milestones 8.1 and 8.2 are `DONE_VERIFIED`. Milestone 8.3 is implemented and its four ledger rows are `IN_PROGRESS` while GitHub browser verification is pending. The exact next checkpoint is Milestone 8.3; complete its mobile and desktop render checks before starting Batch 8.4.
